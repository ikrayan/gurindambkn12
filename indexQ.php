  ...
  <div class="row">
	...
	<div class="card mb-3">
		...
		//if(isset($_SESSION['member_status'])){ //Jika login
			if (isset($_GET['kategori'])){
				$data = mysqli_query($koneksi, "select * from posting,kategori,user where posting_step=2 and posting_member=PNS_ID and kategori_id=posting_kategori ".$bagianWhere);
				...
				$data = mysqli_query($koneksi, "select * from posting,kategori,user where posting_step=2 and posting_member=PNS_ID and kategori_id=posting_kategori order by posting_id DESC");
				$total = mysqli_num_rows($data);
			}
		...
		/*}  else{ //Jika tidak login
			...
			if (isset($_GET['kategori'])){
				$data = mysqli_query($koneksi, "select * from posting,kategori,user where posting_visibility='publik' and posting_step=2 and posting_member=PNS_ID and kategori_id=posting_kategori ".$bagianWhere);
				//echo "select * from posting,kategori,member where posting_visibility='publik' and posting_step=2 and posting_member=member_id and kategori_id=posting_kategori ".$bagianWhere;
				$total = mysqli_num_rows($data);
			}else{
				$data = mysqli_query($koneksi, "select * from posting,kategori,user where posting_visibility='publik' and posting_step=2 and posting_member=PNS_ID and kategori_id=posting_kategori order by posting_id DESC");
				$total = mysqli_num_rows($data);
			}
            ...
            while($d = mysqli_fetch_array($data)){
				...
				// Logika IKON
					$idQ = $d['posting_id'];
					$qattQ = mysqli_query($koneksi, "select * from attachment where attach_posting=$idQ");
					if(mysqli_num_rows($qattQ)>0 && mysqli_num_rows($qattQ)<2){
						while($ikonQ = mysqli_fetch_array($qattQ)){
							$fnQ = $ikonQ['attach_name'];
							$ftQ = pathinfo($fnQ, PATHINFO_EXTENSION);
						}
					}elseif(mysqli_num_rows($qattQ)>1){							
							$ftQ = "multi";
					}
				// end Logika IKON
              ...
              <tr>
                ...
                <td> 
                	<div class="d-flex align-items-start">
                		<i class="fa fa-3x text-black-50 fa-<?php ikon($ftQ); ?>-o mr-2"></i>
						<div class="justify-content-start mt-n1">
						
							<?php
							$fileVideo = array('mp4','mkv','mov','mpg','mpeg','flv','m4v');
								if(in_array($ftQ, $fileVideo)){ ?>
									<a href="video.php?id_posting=<?php echo $d['posting_id']; ?>&video=<?php echo $fnQ; ?>">
							<?php	
								}else{
							?>
									<a href="diskusi.php?id_posting=<?php echo $d['posting_id']; ?>">
							<?php 
								}
							?>
						  ...
						  <div class="badge badge-primary"><i class="fa fa-bookmark"></i>&nbsp;<?php echo $d['kategori_nama'] ?></div>
							...
							if(!empty($d['posting_tags'])){
								...
								for($t=0; $t < count($tags); $t++){
									echo "<a href='indexQ.php?cariTags=".$tags[$t]."&kategori=all'>
											<span class='badge text-black-50 mb-n2 mx-n1'><i class='fa fa-tag'></i>".$tags[$t]."</span>
					  					  </a>";
								}
							}
							?>     		
