...
<div class="container col-xl-9">
	<div class="card mb-4">
		...
		<div class="card-body px-3 shadow">
			...
			$datalatest = mysqli_query($koneksi, "select * from posting, kategori where posting_kategori=kategori_id and posting_step=2 order by posting_id DESC LIMIT 6");

			//}else{ //Jika tidak login
			//	$datalatest = mysqli_query($koneksi, "select * from posting, kategori where posting_kategori=kategori_id and posting_visibility='publik' and posting_step=2 order by posting_id DESC LIMIT 6");
			//}			
			//end logic sql
			...
			?>
				...
				<div class="d-flex flex-wrap ml-n1">
					...
					while ($listlatest = mysqli_fetch_array($datalatest)) {
						...
						// Logika IKON
						$id = $listlatest['posting_id'];
						$qatt = mysqli_query($koneksi, "select * from attachment where attach_posting=$id");
						if (mysqli_num_rows($qatt) > 0 && mysqli_num_rows($qatt) < 2) {
							while ($ikon = mysqli_fetch_array($qatt)) {
								$fnl = $ikon['attach_name'];
								$ftl = pathinfo($fnl, PATHINFO_EXTENSION);
							}
						} elseif (mysqli_num_rows($qatt) > 1) {
							$ftl = "multi";
						}
						// end Logika IKON
					?>
						<div class="d-flex col-lg-4 mb-4 align-items-start">
							<?php
							if ($listlatest['posting_jenis'] == 'forum') {
							?>
								<i class="fa fa-2x text-black-50 fa-forumbee mr-2 mt-2"></i>
							<?php
							} else {
							?>
								<i class="fa fa-2x text-black-50 fa-<?php ikon($ftl); ?>-o mr-2 mt-2"></i>
							<?php
							}
							...
							<div class="justify-content-start pl-2 pt-0" style="border-left: 1px solid grey;">
								...
								<a href="indexQ.php?kategori=<?php echo $listlatest['kategori_id']; ?>">
									<span class="badge text-black-50 mx-n1"><i class="fa mr-1 fa-bookmark"></i><?php echo $listlatest['kategori_nama']; ?></span>
								</a>
								<br>
								<?php
								$fileVideo = array('mp4', 'mkv', 'mov', 'mpg', 'mpeg', 'flv', 'm4v');
								if (in_array($ftl, $fileVideo)) { ?>
									<a href="video.php?id_posting=<?php echo $listlatest['posting_id']; ?>&video=<?php echo $fnl; ?>">
									<?php
								} else {
									?>
										<a href="diskusi.php?id_posting=<?php echo $listlatest['posting_id']; ?>">
										<?php
									}
										?>
				...
				<a href="indexQ.php?kategori=all" class="text-black-50 d-flex justify-content-end"><small>Tampilkan semua >></small></a>
			<?php
			} else {
...
<div class="container col-xl-9">
		...
		<div class="col-xl-9">
				...
				if($ext=="mp4" || $ext=="mkv" || $ext=="mpg" || $ext=="mpeg" || $ext=="mov"){ */
							?>
								<div class="col-lg-4 mb-4">
									<a href="video.php?id_posting=<?php echo $listvideo['posting_id']; ?>&video=<?php echo $listvideo['attach_name']; ?>">
						...
						<a href="indexQ.php?tipeFile=<?php echo $isVideo; ?>&kategori=all" class="text-black-50 d-flex justify-content-end"><small>Tampilkan semua >></small></a>
					<?php
					} else {
			...
			?>
				...
				<div class="card-body px-3 shadow">
					...
					//if(isset($_SESSION['member_status'])){ //Jika login
					$datashare = mysqli_query($koneksi, "select * from posting where posting_kategori='0' and posting_step=2 order by posting_id DESC LIMIT 6");

					//}else{ //Jika tidak login
					//	$datashare = mysqli_query($koneksi, "select * from posting where posting_kategori='0' and posting_visibility='publik' and posting_step=2 order by posting_id DESC LIMIT 6");
					//}			
					//end logic sql
					...
					?>
						<div class="d-flex flex-wrap">
							...
							while ($listshare = mysqli_fetch_array($datashare)) {
								...
								// Logika IKON
								$id2 = $listshare['posting_id'];
								$qatt2 = mysqli_query($koneksi, "select * from attachment where attach_posting=$id2");
								if (mysqli_num_rows($qatt2) > 0 && mysqli_num_rows($qatt2) < 2) {
									while ($ikon2 = mysqli_fetch_array($qatt2)) {
										$fn = $ikon2['attach_name'];
										$ft = pathinfo($fn, PATHINFO_EXTENSION);
									}
								} elseif (mysqli_num_rows($qatt2) > 1) {
									$ft = "multi";
								}
								// end Logika IKON
							...
							?>
								<div class="d-flex align-items-center col-lg-6 mb-4">
									<i class="fa fa-2x text-black-50 fa-<?php ikon($ft); ?>-o mr-2"></i>
									<div class="justify-content-start">
										<?php
										//$fileVideo = array('mp4','mkv','mov','mpg','mpeg','flv','m4v');
										if (in_array($ft, $fileVideo)) { ?>
											<a href="video.php?id_posting=<?php echo $listshare['posting_id']; ?>&video=<?php echo $fn; ?>">
											<?php
										} else {
											?>
												<a href="diskusi.php?id_posting=<?php echo $listshare['posting_id']; ?>">
												<?php
											}
												?>
												<!--<a href="diskusi.php?id_posting=<?php //echo $listshare['posting_id']; 
																					?>">--><span class="text-black-50"><?php echo strtoupper($listshare['posting_judul']); ?></span>
												</a>
						...
						<a href="indexQ.php?kategori=0" class="text-black-50 d-flex justify-content-end"><small>Tampilkan semua >></small></a>
					<?php
					} else {
						echo "<small><i>Belum ada data</i></small>";
					}
			...
			<div class="card mb-4">
				...
				<div class="card-body px-3 shadow">
					...
					<?php
					//if(isset($_SESSION['member_status'])){ //Jika login
					$dataimg = mysqli_query($koneksi, "select * from posting where posting_judul like '%INFOGRAFIS%' and posting_step=2 order by posting_id DESC LIMIT 8");

					//}else{ //Jika tidak login
					//	$dataimg = mysqli_query($koneksi, "select * from posting where posting_judul like '%INFOGRAFIS%' and posting_visibility='publik' and posting_step=2 order by posting_id DESC LIMIT 8");
					//}			
					//end logic sql
					...
					?>
						...
						<div class="d-flex flex-wrap">
							...
							while ($listimg = mysqli_fetch_array($dataimg)) {
								...
								// Logika IKON
								$id3 = $listimg['posting_id'];
								$qatt3 = mysqli_query($koneksi, "select * from attachment where attach_posting=$id3");
								if (mysqli_num_rows($qatt3) > 0 && mysqli_num_rows($qatt3) < 2) {
									while ($ikon3 = mysqli_fetch_array($qatt3)) {
										$fni = $ikon3['attach_name'];
										$fti = pathinfo($fni, PATHINFO_EXTENSION);
									}
								} elseif (mysqli_num_rows($qatt3) > 1) {
									$fti = "multi";
								}
								// end Logika IKON
							...
							?>
								<div class="d-flex align-items-center col-lg-6 mb-4">
									<i class="fa fa-2x text-black-50 fa-<?php ikon($fti); ?>-o mr-2"></i>
									<div class="justify-content-start">
										<a href="diskusi.php?id_posting=<?php echo $listimg['posting_id']; ?>"><span class="text-black-50"><?php echo strtoupper($listimg['posting_judul']); ?></span></a>
						...
						<a href="indexQ.php?tipeFile=infografis&kategori=all" class="text-black-50 d-flex justify-content-end"><small>Tampilkan semua >></small></a>
					<?php
					} else {
						...
						<div id=<?php echo $k['kategori_id']; ?>>
							...
							<div id=<?php echo "katContent" . $lis ?>>
								<ul class="nav nav-tabs ml-3" id="myTab" role="tablist">
									...
									<!-- tab media 
				  <li class="nav-item">
					<a class="nav-link" id=<?php //echo "media-tab".$lis; 
											?> data-toggle="tab" href=<?php //echo "#media".$lis; 
																		?> role="tab" aria-controls="media" aria-selected="false"><small><b>Media</b></small></a>
				  </li> -->
									<!-- tab faq -->
									<li class="nav-item">
										...
										?>
											<div class="d-flex flex-wrap ml-3 mt-4">
												...
												while ($listreg = mysqli_fetch_array($datareg)) {
													...
													// Logika IKON
													$idreg = $listreg['posting_id'];
													$qattreg = mysqli_query($koneksi, "select * from attachment where attach_posting=$idreg");
													if (mysqli_num_rows($qattreg) > 0 && mysqli_num_rows($qattreg) < 2) {
														while ($ikonreg = mysqli_fetch_array($qattreg)) {
															$fn1 = $ikonreg['attach_name'];
															$ft1 = pathinfo($fn1, PATHINFO_EXTENSION);
														}
													} elseif (mysqli_num_rows($qattreg) > 1) {
														$ft1 = "multi";
													}
													// end Logika IKON
												...
												?>
													<div class="d-flex align-items-center col-lg-6 mb-3">
														<i class="fa fa-2x text-black-50 fa-<?php ikon($ft1); ?>-o mr-2"></i>
														<div class="justify-content-start">
															<a href="diskusi.php?id_posting=<?php echo $listreg['posting_id']; ?>"><span class="text-black-50"><?php echo strtoupper($listreg['posting_judul']); ?></span></a>
											...
											<a href="indexQ.php?kategori=<?php echo $k['kategori_id']; ?>&jenis=regulasi" class="text-black-50 d-flex justify-content-end"><small>Tampilkan semua >></small></a>
										<?php
										} else {
										...
										?>
											<div class="d-flex flex-wrap ml-3 mt-4">
												...
												while ($listdok = mysqli_fetch_array($datadok)) {
													...
													// Logika IKON
													$iddok = $listdok['posting_id'];
													$qattdok = mysqli_query($koneksi, "select * from attachment where attach_posting=$iddok");
													if (mysqli_num_rows($qattdok) > 0 && mysqli_num_rows($qattdok) < 2) {
														while ($ikondok = mysqli_fetch_array($qattdok)) {
															$fn2 = $ikondok['attach_name'];
															$ft2 = pathinfo($fn2, PATHINFO_EXTENSION);
														}
													} elseif (mysqli_num_rows($qattdok) > 1) {
														$ft2 = "multi";
													}
													// end Logika IKON
												...
												?>
													<div class="d-flex align-items-center col-lg-6 mb-3">
														<i class="fa fa-2x text-black-50 fa-<?php ikon($ft2); ?>-o mr-2"></i>
														<div class="justify-content-start">
															<?php
															$fileVideo = array('mp4', 'mkv', 'mov', 'mpg', 'mpeg', 'flv', 'm4v');
															if (in_array($ft2, $fileVideo)) { ?>
																<a href="video.php?id_posting=<?php echo $listdok['posting_id']; ?>&video=<?php echo $listdok['attach_name']; ?>">
																<?php
															} else {
																?>
																	<a href="diskusi.php?id_posting=<?php echo $listdok['posting_id']; ?>">
																	<?php
																}
																	?>
											...
											<a href="indexQ.php?kategori=<?php echo $k['kategori_id']; ?>&jenis=knowledge" class="text-black-50 d-flex justify-content-end"><small>Tampilkan semua >></small></a>
										<?php
										} else {
											echo "<br><span class='pl-4'><small><i>Belum ada data</i></small></span>";
										}
	...
	<?php
	include 'footer.php';
	include 'assets_forum/loopingKategoriIndex.php';
	?>
